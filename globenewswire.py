...
def get_globenewswire_news(symbol):
    # Agregar un parámetro único a la URL para evitar el caché (timestamp)
    url = f"https://www.globenewswire.com/en/search/keyword/{symbol}?timestamp={int(time.time())}"
    headers = {
        "User-Agent": "Mozilla/5.0",
        "Cache-Control": "no-cache",  # Instrucción para no usar caché
        "Pragma": "no-cache"  # Otra instrucción para evitar caché
    }
...
# print(get_globenewswire_news("GWRS"))