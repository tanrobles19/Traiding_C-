...
def create_subscription_list(tickers):
    subscriptions = [f"A.{ticker}" for ticker in tickers]    
...
def buyStock(ib, PORT, symbol, second, active_seconds_count, trade_signals_count, polygon_second_close, real_time_positions, investment_amount, open_map, open_map_timestamp, INCREASE_FROM_OPEN, MIN_PRICE_THRESHOLD, MAX_PRICE_THRESHOLD, LOW_FLOAT_THRESHOLD, PRICE_SPIKE_RISK_THRESHOLD, last_trade_price, trade_signals_track_stock_price, cumulative_volume):
    ...
    #Bug con el second... este second sale del ultimo trade que nos llego, pero cuando hay un delay en apertura de mercado podria ser el segundo 59 y nos esta llegando el segundo 
    # 1... eso esta mal... Hay qye tomar el segundo local que es el que importa. 
    
    # Si el trade del que extraemos el second llego retrasado el calculo se hace mal. Con current_time.second eso ya no pasa. Si estamos sincronizados o no se calcula bien.
    ...
    if last_trade_price > open_map[symbol]: # Check if the last trade price is greater than the open price
        ...
        if price_increase_percentage >= INCREASE_FROM_OPEN:
            ...
            if percentage_increase < PRICE_SPIKE_RISK_THRESHOLD:
                ...
                loop.run_until_complete(run_async_tasks(symbol, stock, order, real_time_positions))        
...
async def run_async_tasks(symbol, stock, order, real_time_positions):  
    ...
    print("Done!")
    print(f"Bought {symbol} at {trade.orderStatus.avgFillPrice} - Status: {trade.orderStatus.status}")
    real_time_positions[symbol] = trade.orderStatus.avgFillPrice
...
def market_data_producer(queue, symbols: List[str]):
    ...
    def handle_msg(msgs):
        ...
        for m in msgs:
            ...
            #     print(f" \033[1;31m$WebSocket Active\033[0m")
            #     print(last_time)
            #     print(" ")
    ...
    while True:
        try:
            ...
            if temp_time_stamp < current_minute or ( current_minute == 0 and temp_time_stamp == 59):
                ...
                if trade_volume is not None:
                    ...
                    if symbol in real_time_positions:
                        ...
                        if trade_consumer_close_map[symbol] < bot_price * MAX_LOSS_TOLERANCE_PER_TRADE and trade_consumer_close_map[symbol] > 0:     
                            ...
                            print(f"Current price: {trade_consumer_close_map[symbol]}, Purchase price: {bot_price}")
                        ...
                        if trade_consumer_close_map[symbol] >= bot_price * 1.1:
                ...
                if symbol in high_short_interest_stocks:    
                    ...
                    if trade_consumer_open_map[symbol] > 0:
                        ...
                        if price_change_percentage > ( increase_open + 2 ) and symbol not in trade_signals_memory_hashmap:
                            ...
                            cumulative_volume, -1, {}, relative_volume_factor_hour, increase_open, 0, trade_id, trade_exchange, trade_trf_id, "short_squeeze" )                                                    
                ...
                if symbol in trade_signals_track_stock_price:
                    ...
                    if percentage_change >= increase_open:
                        ...
                        tradeSignalId, purchasePrediction = persist_trade_signal(mysql_cursor, db_connection, symbol, consumer_id, active_seconds_count, trade_per_minute_map[symbol], unix_timestamp, trade_consumer_open_map[symbol], trade_consumer_open_map_timestamp[symbol], trade_consumer_close_map[symbol], cumulative_volume, 10, stock_float_hashmap[symbol],
                        cumulative_volume, -1, {}, relative_volume_factor_hour, increase_open, 0, trade_id, trade_exchange, trade_trf_id, "track_stock_price_increase" )                                                 
                ...
                if relative_volume_factor_hour >= RELATIVE_VOLUME_FACTOR:
                    ...
                    if symbol not in trade_signals_memory_hashmap:    
                        ...
                        if has_existing_trade_signal_with_News(mysql_cursor, symbol) == False:
                            ...
                            tradeSignalId, purchasePrediction = persist_trade_signal(mysql_cursor, db_connection, symbol, consumer_id, active_seconds_count, trade_per_minute_map[symbol], unix_timestamp, trade_consumer_open_map[symbol], trade_consumer_open_map_timestamp[symbol], trade_consumer_close_map[symbol], cumulative_volume, 10, stock_float_hashmap[symbol],
                            cumulative_volume, -1, {}, relative_volume_factor_hour, increase_open, 0, trade_id, trade_exchange, trade_trf_id, "----")                               
            ...
            if( current_minute != datetime.now().minute ):    
                ...
                # print(f"Real Time Positions Updated : {len(real_time_positions)}")
                print("----------------------")        
                print("   ")        
                old_trades_timestamp = 0
...
def run_processes(symbols, ws, port, trade_queue, high_short_interest_stocks, low_float_stocks_dict, stock_float_hashmap, relative_volume_low_float, trade_signal_limit, stale_threshold_minutes_config, increase_open, low_float_threshold, min_price_threshold, max_price_threshold, price_spike_risk_threshold, investment_amount, RELATIVE_VOLUME_THRESHOLD, short_interest_ratio, max_loss_tolerance_per_trade, news_time_window_minutes, relative_volume_factor):
    ...
    with multiprocessing.Manager() as manager:
        ...
        process2 = multiprocessing.Process(target=consumer_process, args=(ib_zero, trade_queue, trade_signal_queue, news_queue, selloff_queue, take_profit_queue, high_short_interest_stocks, symbols, "worker-id", port, stock_float_hashmap, low_float_stocks_dict, relative_volume_low_float, trade_signal_limit, stale_threshold_minutes_config, increase_open, low_float_threshold, RELATIVE_VOLUME_THRESHOLD, trade_signals_track_stock_price, real_time_positions, max_loss_tolerance_per_trade, relative_volume_factor))
...
def main():
    ...
    RELATIVE_VOLUME_THRESHOLD = 5          # This is the threshold for relative volume to trigger a trade. Ross Cameron recommends at least 5x relative volume.    
    TRADE_CAPITAL = 1000                      # This is the budget allocated for a single trade.
    ...
    MIN_PRICE_THRESHOLD = 0.5                  # Min price limits for trade validation. Price range used to filter valid assets after a TradeSignal(BUY). Also used to build the low_float dictionary.
    MAX_PRICE_THRESHOLD = 30                  # Max price limits for trade validation. Price range used to filter valid assets after a TradeSignal(BUY). Also used to build the low_float dictionary.
...
# print(( bot_price * (1 + range_sell / 100)))
main()